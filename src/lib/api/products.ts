...
export async function getBrands(): Promise<Brand[]> {
  ...
  const { data, error } = await supabase.from("brands").select("brand_id, brand_name").order("brand_name", { ascending: true });
...
export async function getProductById(productId: number): Promise<EditableProduct> {
  ...
  return {
    ...data,
    switch_ids: (data.product_switches ?? []).map((ps) => ps.fk_switch_id),
  } as EditableProduct;
}
